...
function add_match_score_custom_user_profile_fields( $user ) {
    
 $match_card= get_the_author_meta( 'MatchCard', $user->ID );
...
?>
	...
	<table class="form-table">
		<tr>
			<th>
				<label for="MatchCard"><?php _e('Show Match Form', 'your_textdomain'); ?>
			</label></th>
			<td>
                            <input type="checkbox"   name="MatchCard" id="MatchCard" value="yes"  <?php if(!empty($match_card)){ echo 'checked="checked"'; } ?>   /><br />
...
<?php }
 ...
 function  save_match_score_custom_user_profile_fields( $user_id ) {
	...
	if ( !current_user_can( 'edit_user', $user_id ) )
        ...
        $user_role='';
        $user = new WP_User( $current_user_login->ID );
	...
	update_usermeta( $user_id, 'MatchCard', $_POST['MatchCard'] );
}
...
if(!empty($user_role)){        
add_action( 'show_user_profile', 'add_match_score_custom_user_profile_fields' );
add_action( 'edit_user_profile', 'add_match_score_custom_user_profile_fields' );

add_action( 'personal_options_update', 'save_match_score_custom_user_profile_fields' );
add_action( 'edit_user_profile_update', 'save_match_score_custom_user_profile_fields' );
}

//echo get_the_author_meta( 'MatchCard', 8 );
