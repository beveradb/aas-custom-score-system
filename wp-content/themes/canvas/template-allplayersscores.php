<?php
/**
 * Template Name: All Players Scores
 *
 ...
 */
 ...
 if(isset($_POST['matchid'])) {
	...
	?>
	This value is from match ID <?=$matchID?>:<br /><br />
	...
	<em>This match data was directly entered using the match card page </em><?=$match->date?><br />
							...
							foreach($sections as $section) { ?>
								...
								foreach($teams as $team) { ?>
									<table class="allScores">
										<tr>
											<th colspan="21" class="teamName"><?=stripslashes($team->name)?></th>
											<th class="winHeading"></th>
											<th class="drawHeading"></th>
											<th class="loseHeading"></th>
										</tr>
										<tr>
											<th class="pointsHeading">Team Points</th>
											<? 	for($i=1; $i<=18; $i++) { 
													$matchFound = false;
										...
										<? $players = $wpdb->get_results("SELECT * FROM {$tablePrefix}players WHERE team_id = '{$team->id}' AND name NOT LIKE 'Handicap' ");
