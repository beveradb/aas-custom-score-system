...
if (!$columns && support("table")) {
	...
	if (!$result) {
		...
		if (($rows || $page) && !js_adminer_ajax()) {
				...
				if ($jush != "simpledb") {
					...
					if ($max_page > 0) {
						...
						echo ($exact_count && $found_rows !== false
							? adminer_pagination($max_page, $page)
					...
					echo lang('Page') . ":";
					echo adminer_pagination(0, $page) . ($page > 1 ? " ..." : "");
					echo ($page ? adminer_pagination($page, $page) : "");
					echo ($max_page > $page ? adminer_pagination($page + 1, $page) . ($max_page > $page + 1 ? " ..." : "") : "");
				}
			}
			...
			echo adminer_checkbox("all", 1, 0, lang('whole result'), "var checked = formChecked(this, /check/); selectCount('selected', this.checked ? '$display_rows' : checked); selectCount('selected2', this.checked || !checked ? '$display_rows' : checked);") . "\n";
